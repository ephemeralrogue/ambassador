...
import OAuth1Emissary from './oauth1emissary.js';
import sessionStore from '../state/oauth1session.js';
...
const OAuth1Delegate = Object.create(OAuth1Emissary,
{
  ...
  ***********************************************************************/
   ...
   * exchange. The following options are required to build the OAuth object and populate
   * this embassy with the proper credentials:
   *     requestTokenURL
   *     accessTokenURL
   *     consumerKey
   *     consumerSecret
  ...
  _populateOAuth1Delegate:
  {
    value: function populateOAuth1Delegate(options, verify)
    {
      ...
      // NOTE: The _oauth property is considered "protected".  Subclasses
      // are allowed to use it when making protected resource requests to
      // retrieve the user profile.
      
      this._populateOAuth1Emissary(options, verify);
