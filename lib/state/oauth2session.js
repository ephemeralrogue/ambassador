  ...
  _verifyState:
  {
    value: function verifyState(req)
    {
      ...
      return new Promise((resolve, reject) =>
      {
        ...
        {
          case !req.session:
            verifyState.err = new Error('OAuth 2.0 authentication requires session support when using state. Use express-session middleware?');
            return reject(verifyState);
          /*case !req.session[key]:
            verifyState.err = { message: 'Unable to verify authorization request state.' };
            return reject(verifyState);*/
          case !state:
            verifyState.err = { message: 'Unable to verify authorization request state.' };
            return reject(verifyState);
          case state !== returnedState:
            verifyState.err = { message: 'Invalid authorization request state.' };
            return reject(verifyState);
          default:
            stately.err = null;
            stately.ok = true;
            // stately.state = req.session[key].state
